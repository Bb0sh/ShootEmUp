...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            # Check for F1 key: print README.md to console
            if keystate[pygame.K_F1]:
                with open(os.path.join(game_folder, 'README.md')) as file:
                    data = file.read()
                    print(data)
            ...
            if keystate[pygame.K_F3]:
                if config.debug == False:
                    ...
                    config.debug = False
            if keystate[pygame.K_F5]:
                print('''Pygame Shoot-em-up version {}, Copyright (C) 2016 Samuel Preston Spence
        Gnomovision comes with ABSOLUTELY NO WARRANTY; for details press F6.
        This is free software, and you are welcome to redistribute it
        under certain conditions; press F6 for details.'''.format(VERSION))

            if keystate[pygame.K_F6]:
                try:
                    with open(os.path.join(game_folder, 'LICENSE')) as file:
                        data = file.read()
                        print(data)
                except IOError:
                    print('Warning! LICENSE does not exist!\nGo to https://github.com/thespecialbro/Shoot-em-up to download the necessary file(s)')
